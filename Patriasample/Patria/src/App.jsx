    ...
    <div className="account-dashboard open react-account-dashboard" id="accountDashboard">
      <div className="container">
        <div className="account-dashboard-grid">
          <aside className="account-side" id="accountSideMenu">
          ...
          <main className="account-main">
            ...
            {page === "shipping" && <section className="account-page active"><h2>物流 Shipping</h2><p className="account-main-desc">查看取貨日期、訂單處理狀態與配送地址。</p><section className="account-section"><h3>目前取貨地址</h3><div className="shipping-address-card"><strong>{addressForm.fullName || user.name || "Member"}</strong><span>{addressForm.phone || user.phone || "尚未設定電話"}</span><span>{addressLabel}</span><button type="button" onClick={() => setPage("addresses")}>編輯地址</button></div></section><section className="account-section"><h3>物流／取貨進度</h3>{orders.length ? orders.map((order) => <div className="shipping-order-row" key={order.id}><div><strong>Order #{order.id}</strong><span>Pickup date: {order.fulfillmentDate || "—"}</span></div><OrderStatus status={order.status} /></div>) : <div className="account-empty-row"><span>目前沒有物流訂單。</span></div>}</section></section>}
